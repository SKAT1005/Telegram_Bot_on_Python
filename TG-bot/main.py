...
def get_number_photo(message, lst, chat_id):
    """Функция, принмает кол-во фотогравий
    отеля, которые хочет посмотреть пользователь.
    ...
    if 0 < int(x) <= max_photo:
        lst.append(x)
        if lst[0] == 'lp':
            lowprice(lst[1], lst[2], lst[3],
                     str(lst[4]), int(lst[5]),
                     int(lst[6]), chat_id)
        elif lst[0] == 'hp':
            hightprice(lst[1], lst[2], lst[3],
                       str(lst[4]), int(lst[5]),
                       int(lst[6]), chat_id)
        else:
            bestdeal(lst[1], lst[2], lst[3],
                     str(lst[4]), int(lst[5]), int(lst[6]),
                     int(lst[7]), int(lst[8]),
                     int(lst[9]), int(lst[10]), chat_id)
    else:
        msg = bot.send_message(message.chat.id, "Введите число, "
                                                "которое больше 0 и меньше "
                                                "{number}"
                               .format(number=max_photo))
        bot.register_next_step_handler(msg, get_number_photo, lst, chat_id)


def ask_about_photo(message, lst, chat_id):
    """Функция, принмает ответ от
    пользователя хочет он посмотреть фотографии отеля или нет.
    ...
    if x == 'Да' or x == 'да':
        msg = bot.send_message(message.chat.id,
                               'Введите сколько фотографий отелей хотите'
                               'посмотреть(больше 0 и меньше {number})'
                               .format(number=max_photo))
        bot.register_next_step_handler(msg, get_number_photo, lst, chat_id)
    elif x == 'Нет' or x == 'нет':
        if lst[0] == 'lp':
...
def get_number_hotel(message, lst, chat_id):
    """Функция, для определения кол-ва отелей,
    которые хчет посмотреть пользователь.
    ...
    if x in '0123456789':
        if 0 < int(x) <= max_hotels:
            ...
            msg = bot.send_message(message.chat.id,
                                   ...
                                   "Введите 'Да', чтобы посмотреть и"
                                   "'Нет', если не хотите смотреть")
            bot.register_next_step_handler(msg, ask_about_photo, lst, chat_id)
        else:
            msg = bot.send_message(message.chat.id,
                                   "Введите число, которое больше 0"
                                   "и меньше либо равно {number}"
                                   .format(
                                       number=max_hotels))
            bot.register_next_step_handler(msg, get_number_hotel, lst, chat_id)
    else:
        msg = bot.send_message(message.chat.id,
                               "Введите число, которое больше 0 и"
                               "меньше либо равно {number}"
                               .format(number=max_hotels))
        bot.register_next_step_handler(msg, get_number_hotel, lst, chat_id)


def get_distance_range(message, lst, chat_id):
    """Функция, для определения промежутка
    допустимой дистанции от отлея до центра.
    ...
    if len(x) == 2 and 0 <= int(x[0]) < int(x[1]):
        ...
        lst.append(int(x[1]))
        msg = bot.send_message(message.chat.id,
                               "Введите сколько отелей хотите посмотреть"
                               "(больше 0 и меньше либо равное {number})"
                               .format(
                                   number=max_hotels))
        bot.register_next_step_handler(msg, get_number_hotel, lst, chat_id)
    else:
...
def get_price_range(message, lst, chat_id):
    ...
    if len(x) == 2 and 0 <= int(x[0]) < int(x[1]):
        ...
        msg = bot.send_message(message.chat.id,
                               ...
                               "(расстояния должны быть больше 0,"
                               " разделены '-' и первое число"
...
def get_check_in_and_check_out(message, lst, chat_id):
    """Функция, для определения даты пребывания
    пользователя в отеле. После завершения вызывает функцию get_city
    ...
    if len(a) != 2:
        msg = bot.send_message(message.chat.id,
                               "Даты должны разделяться"
                               "пробелом и иметь формат (YYYY-MM-DD).")
        bot.register_next_step_handler(msg,
                                       get_check_in_and_check_out,
                                       lst, chat_id)
    else:
        a0 = a[0].split('-')
        a1 = a[1].split('-')
        if len(a0[2]) == 2 and len(a0[1]) == 2 and len(a0[0]) == 4 \
            ...
            if int(a0[1]) <= 12 or int(a1[1]) <= 12:
                if len(a0[0]) != 4 or len(a0[1]) != 2 or len(a0[2]) != 2 \
                        or len(a1[0]) != 4 or len(a1[1]) != 2 \
                        or len(a1[1]) != 2:
                    msg = bot.send_message(message.chat.id,
                                           "Введите период на который"
                                           "вы хотите поехат в формате"
                                           "(YYYY-MM-DD). Если месяц или день "
                                           "заканчивается на 1 цифру,"
                                           "то перед ней поставьте"
                                           "0 Например:\n"
                ...
                if int(a0[0]) == int(a1[0]) and int(a0[1]) == int(a1[1]):
                    ...
                    if int(a0[1]) == 1 or int(a0[1]) == 3 or int(a0[1]) == 5 \
                            or int(a0[1]) == 7 \
                            or int(a0[1]) == 8 or int(a0[1]) == 10 \
                            or int(a0[1]) == 12:
                        c = 31 - int(a0[2]) + int(a1[2])
                        lst.append(a[0])
                        lst.append(a[1])
                        lst.append(c)
                    elif int(a0[1]) == 4 or int(a0[1]) == 6 \
                            or int(a0[1]) == 9 or int(a0[1]) == 11:
                        ...
                        lst.append(c)
                msg = bot.send_message(message.chat.id,
                                       "Введите город, в который"
                                       "вы хотите поехать на английском языке."
                                       " Поиск по России временно недоступен.")
                bot.register_next_step_handler(msg, get_city, lst, chat_id)
            else:
                msg = bot.send_message(message.chat.id,
                                       "Месяцев только 12, введите число,"
                                       "которое больше нуля"
                                       "и меньше или равно 12")
                bot.register_next_step_handler(msg,
                                               get_check_in_and_check_out,
                                               lst, chat_id)
        else:
            msg = bot.send_message(message.chat.id,
                                   "Месяц и день должны содержать"
                                   "2 цифры, а год 4 цифры"
                                   "Если месяц равняется 1,"
                                   "но поставьте перед ним 0. Например:\n"
...
def start(message):
    """
    Метод для создания создания кнопок бота.
    После создания всех кнопок вызывает метод get_text_messages
    ...
    bot.send_message(message.chat.id,
                     text="Привет, {0.first_name}! Спасибо,"
                          "что пользуетесь услугами нашей компании"
                     .format(
...
def get_text_messages(message):
    ...
    /hightprice -- Создаёт список lst,
    которая содержит краткое название команды,
    создает переменную chat_id, которая содержит id чата с пользователем,
    cоздает переменную msg, которая содержит ответ пользователя
    и в функцию get_check_in_and_check_out передет 3 аргумента
    1 -- Сообщение пользователя
    ...
    /history  -- Создает переменную
    chat_id, которая содержит id чата с пользователем
    ...
    if message.text == "/help":
        ...
        bot.send_message(message.from_user.id,
                         "Напишите /lowprice,"
                         "чтобы узнать информацию о самых дешевых отелях\n"
                         "/hightprice для того,"
                         "чтобы узнать о самых дорогих отелях\n"
                         "/bestdeal для того,"
                         "чтобы указать диапазон цен и расстояния"
                         " и увидеть подходящие вам отели,\n"
                         "/history для того,"
                         "чтобы узнать о истории своих запросов.")
        ...
        msg = bot.send_message(message.chat.id,
                               "Введите период на который"
                               "вы хотите поехать в формате"
                               "(YYYY-MM-DD YYYY-MM-DD) если месяц или день "
                               "заканчивается на 1 цифру,"
                               "то перед ней посавьте 0 Например:\n"
                               "2022-9-1 2022-10-1 Неправильно\n"
                               "2022-09-01 2022-10-1 Правильно")
        bot.register_next_step_handler(msg,
                                       get_check_in_and_check_out,
