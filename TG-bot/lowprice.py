...
def lowprice(checkindate: str, checkoutdate: str,
             day: int, city: str, number_hotel:
             int, number_photo,
             chat_id: int) -> None:
        ...
        with open('history.text', 'a') as history:
            ...
            try:
                ...
                for i in range(number_hotel):
                    lst.append(a[i]['name'])
                    bot.send_message(chat_id, "Отель номер {number}"
                                     .format(number=i + 1))
    ...
    with open('history.text', 'a', encoding='utf-8') as history:
        history.write(
            ...
            '/lowprice|Найденные отели: {hotels}\n'.format(
