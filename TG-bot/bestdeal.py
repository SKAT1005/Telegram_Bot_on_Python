...
def bestdeal(checkindate: str, checkoutdate: str, day: int, city: str, min_price: int, max_price: int,
             min_distance: int, max_distance: int, number_hotel: int,
             number_photo, chat_id) -> None:
    """Функция выводящяя Названия, адресса, сайты, расстояния до центра,
    цены за ночь, цены за весь период и фотографии отелей по фильтрам пользователя
    ...
    querystring = {"destinationId": get_region(city), "pageNumber": "1", "pageSize": "200", "checkIn": checkindate,
                   "checkOut": checkoutdate, "adults1": "1", "sortOrder": "PRICE", "locale": "en_US",
                   "currency": "USD"}
    ...
    with open('lowprice.json') as json_file:
        ...
        try:
            ...
            for i in range(number_hotel):
                lst.append(a[i]['name'])
                if min_price <= a[i]['ratePlan']['price']['exactCurrent'] <= max_price \
                        and min_distance <= float(a[i]['landmarks'][0]['distance'][:3]) <= max_distance:
                    b += 1
                    lst.append(a[i]['name'])
    ...
    if b >= 1:
        with open('history.text', 'a', encoding='utf-8') as history:
            history.write(
                '{id}|Дата выполнения запроса: {date}|Команда: /bestdeal|Найденные отели: {hotels}\n'.format(
                    id=chat_id,
                    date=datetime.date.today(),
                    hotels=", ".join(lst)))
    else:
        ...
        with open('history.text', 'a', encoding='utf-8') as history:
            history.write(
                '{id}|Дата выполнения запроса: {date}|Команда: /bestdeal|'
                'Найденные отели: К сожалению отелей найдено не было\n'.format(
                    id=13314,
                    date=datetime.date.today()))
